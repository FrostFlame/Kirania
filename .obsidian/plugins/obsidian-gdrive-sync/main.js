...
var getFilesList = async (accessToken, vault) => {
  try {
    const response = await (0, import_obsidian.requestUrl)({
      url: "https://www.googleapis.com/drive/v3/files" + (vault != null ? `?q='${vault}'%20in%20parents&fields=files(name, modifiedTime, mimeType, id),nextPageToken&pageSize=1000` : ""),
      method: "GET",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        Accept: "application/json"
    ...
    while (isNextPageAvailable) {
      const response2 = await (0, import_obsidian.requestUrl)({
        url: "https://www.googleapis.com/drive/v3/files" + (vault != null ? `?q='${vault}'%20in%20parents&fields=files(name, modifiedTime, mimeType, id),nextPageToken&pageSize=1000` : "") + `&pageToken=${nextPageToken}`,
        method: "GET",
        headers: {
          Authorization: `Bearer ${accessToken}`,
          Accept: "application/json"
...
var getFoldersList = async (accessToken, vault = null) => {
  try {
    const response = await (0, import_obsidian.requestUrl)({
      url: "https://www.googleapis.com/drive/v3/files?q=mimeType%20%3D%20'application%2Fvnd.google-apps.folder'" + (vault != null ? `%20and%20'${vault}'%20in%20parents` : "") + "&fields=files(name, id),nextPageToken&pageSize=1000",
      method: "GET",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        Accept: "application/json"
    ...
    while (isNextPageAvailable) {
      const response2 = await (0, import_obsidian.requestUrl)({
        url: "https://www.googleapis.com/drive/v3/files?q=mimeType%20%3D%20'application%2Fvnd.google-apps.folder'" + (vault != null ? `%20and%20'${vault}'%20in%20parents` : "") + `&fields=files(name, id),nextPageToken&pageSize=1000&pageToken=${nextPageToken}`,
        method: "GET",
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      ...
      try {
        ...
        if (toDownload.length) {
          ...
          for (const dFile of toDownload) {
            ...
            await this.app.vault.createBinary(file[0], file[1]).catch(async () => {
...
`));
    ...
    this.getLatestContent = async (file, forced = false) => {
      try {
        ...
        if (this.cloudFiles.includes(file == null ? void 0 : file.path) && !this.syncQueue) {
          ...
          if (forced == "forced" || isBinaryFile && parseInt(this.settings.autoRefreshBinaryFiles) || timeStamp && cloudDate.getTime() > new Date(timeStamp[0]).getTime() + 3e3) {
            ...
            if (this.syncQueue && !isBinaryFile)
              return;
  ...
  async onload() {
    ...
    this.registerEvent(this.app.vault.on("modify", async (e) => {
      ...
      try {
        ...
        this.syncQueue = true;
        if (!(e instanceof import_obsidian2.TFile) || this.writingFile || e.extension != "md") {
          return;
        }
        if (this.timer) {
